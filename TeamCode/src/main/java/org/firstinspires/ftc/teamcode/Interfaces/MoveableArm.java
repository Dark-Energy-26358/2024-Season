...
public interface MoveableArm {
    ...
    void moveTowardsTargetArmExtension(int targetExtension); //set your target extension, the arm will go to that position. this should be in inches.
    ...
    int getTargetArmRotation(); //gets target arm rotation in degrees

    int getCurrentArmExtension(); //gets current arm extension in inches
    int getCurrentArmRotation(); //gets current arm rotation in degrees
